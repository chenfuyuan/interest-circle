...
public interface CircleUserMapper extends BaseMapper<CircleUser> {
     ...
     * @return
     */
    @Select("select * from circle_user where uid = #{uid} and state!=2 order by update_time desc")
    ...
    @Select("select u.* from User u join circle_user cu on u.id = cu.uid where cu.cid = #{cid} and cu.state!=2 order" +
            " " +
            "by " +
            "type asc")
    ...
    @Update("update circle_user set state = 2 where cid=#{cid} and uid=#{uid}")
